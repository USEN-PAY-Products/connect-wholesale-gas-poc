...
| U-18 | 加盟店不足 | 一部加盟店がCSVに含まれていない | アラート「加盟店（顧客コード: 〇〇 / 〇〇店）のデータがCSVに含まれていません。この加盟店の請求は行われませんが、よろしいですか？」+ **ボタンは有効のまま** |
...
| C-9 | 確認画面で戻る | 「← 一覧に戻る」をクリック | 確認モーダル表示 → 確定でTop画面（`#home`）に遷移 |
...
| C-14 | 再送信で「一覧に戻る」 | 「← 一覧に戻る」確定 | Top画面（`#home`）に遷移。「一覧」はTOPを指すため、再送信モードでも詳細画面には戻らない |
| C-15 | 再送信で「キャンセル」 | サマリーカード内の「キャンセル」確定 | 詳細画面（`#detail?invoiceId=xxx`）に遷移（「一覧に戻る」とは遷移先が異なる） |
...
| 一部加盟店がCSV未含有 | 加盟店（顧客コード: 〇〇 / 〇〇店）のデータがCSVに含まれていません。この加盟店の請求は行われませんが、よろしいですか？ | ⚠ アラートのみ |

### 7.4 確認画面

| 条件 | 期待されるメッセージ | ブロッキング |
|------|-------------------|-------------|
| 備考 251文字以上 | 請求書備考は250文字以内で入力してください。 | ✅ |
