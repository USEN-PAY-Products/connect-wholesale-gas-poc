...
> **目的**: 仕入れコネクト（卸側）の機能全体像・業務フロー・ステータスの流れを把握する

---

## 1. サービス概要

仕入れコネクト は、**卸事業者**が加盟店への請求データをCSVでアップロードし、USEN PAY社による審査・加盟店確認を経て支払いを受けるための業務システムです。
...
| ログアウト | ヘッダー右上の卸事業者名をクリック→ドロップダウンからログアウトし、LPへリダイレクト（全画面共通） |
...
| 受付期間（締切） | 請求書受付期間を過ぎている場合はバナーを表示してアップロードを無効化 |
| 契約終了卸ブロック | 仕入れコネクトの契約が終了（`wholesaler_status=end`）の卸は新規請求不可（ログイン・再請求は可能） |
| 契約終了加盟店の除外 | 契約終了（`store_status=end`）の加盟店は新規請求の対象から除外（詳細画面からの再請求は可能） |
...
| 税内訳の調整 | 加盟店ごとの「税内訳（10%）」「税内訳（8%）」を手動で調整可能（計算値から±1円まで） |
...
stateDiagram-v2
    ...
    未検収 --> 確認中: USEN PAY社が承認し\n加盟店に確認依頼
    未検収 --> 差戻し: USEN PAY社が差戻し

    確認中 --> 承認: 加盟店が承認
    確認中 --> 否認差戻: 加盟店が否認

    差戻し --> 未検収: 修正CSV再アップロード\nor 変更なしで再請求

    否認差戻 --> 未検収: 修正CSV再アップロード\nor 変更なしで再請求
    否認差戻 --> 取下げ済み: 請求取り下げ
...
|-----------|------------|--------------|-------------------|
| 未検収 | 「未検収」バッジ | 確認中・承認済み | 不要（USEN PAY社の審査待ち） |
...
| 受付期間（締切） | 請求書受付期間（`WHOLESALER_INVOICE_STORAGE`）を過ぎると新規アップロード不可（バナー表示） |
| 契約終了卸の制限 | `wholesaler_status=end` の卸はログイン・参照・再請求は可能だが、新規請求は不可 |
| 契約終了加盟店の除外 | `store_status=end` の加盟店は新規請求から除外（再請求は詳細画面から可能） |
...
| 異議申立期間制限 | 異議申立期間（`OBJECTION_PERIOD`）終了後は、詳細画面の全対応操作（修正ファイルをアップ / 変更なしで再請求 / 請求取り下げ / CSV一括アップロード / 取下げをやめる）が不可。ボタンが無効化され「異議申立期間が終了しています」と表示 |
