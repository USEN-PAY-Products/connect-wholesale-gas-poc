...
#### `withdrawStoreInvoice` の変更

| Before | After |
|--------|-------|
| `runTransactionSql_()` で実行（影響行数チェックなし） | `runDmlWithRowCheck_()` で実行し `affected === 0` を検証 |
| — | 対象なし時は `return error_(...)` で業務エラーを返却 |
...
function undoWithdrawStoreInvoice(storeInvoiceId, parentInvoiceId) {
  ...
  //     AND is_latest = TRUE AND invoice_status = 'WITHDRAWN'
  // affected === 0 → return error_(...)
  // 成功 → return success_(...)
}
```
...
| 内部関数は末尾 `_` | ✅ `executeWithdraw_` / `executeUndoWithdraw_` / `runDmlWithRowCheck_` |
